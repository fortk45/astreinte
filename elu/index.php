...
//RECUPERE LA CONNEXION SQL
require 'confMySQL.php';

//ON RECUPERE LE NOM DU DOSSIER DANS LEQUEL EST LA MALETTE ELUS
$searchMalette = $connection->query("SELECT `value_param` FROM `parametres` WHERE `nom_param`='dossier_elu'");
$malette = ($searchMalette->fetch())[0];
$nbMalette = strlen($malette);

//RECUPERE LA RECHERCHE DE DOSSIERS
		...
		if (isset($_REQUEST['search_rapport'])){
			...
			if (isset($search)){
				...
				if ((isset($folder)) && (!empty($folder))){
					...
					include 'adm_files/navig_adm.php';
					echo '<div class="row">';
					echo explore_dir_scan_html('../partage/'.$malette);
					echo '</div></div>';
				}
			} 
		}
